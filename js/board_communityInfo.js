...
$(document).ready(function() {
		...
		if ($("#fileIdx").val() > 0) {
			$.post(
				...
				function(fileInfo) {
					...
					$(".delInput1 a").attr(
						"href",
						"/page/downloadData.php?idx=" + fileInfo["idx"]
					);

					$(".dataFileInput").val(
						fileInfo["real_name"] +
							" (" +
							bytesToSize(fileInfo["file_size"]) +
							")"
					);
	...
	$("#board_submit").on("click", function() {
		$("#board_communityInfo_form").submit();
	});
	...
	$("#board_delete").on("click", function() {
		if (confirm("삭제하시겠습니까?")) {
			...
			$.post(
				...
				function(res) {
					if (res.trim() == "success") {
						location.href = "/?page=board_communityList";
					} else {
						alert("처리에 실패하였습니다.");
					}
				}
	...
	dropzone1.ondrop = function(e) {
		...
		$("input[name=dataFile]").replaceWith(
			$("input[name=dataFile]").clone(true)
		);
		$("input[name=dataFile]")[0].files = e.dataTransfer.files;
		var label = e.dataTransfer.files[0].name
			.replace(/\\/g, "/")
			.replace(/.*\//, "");
