$(document).ready(function(){
	...
	var table = $("#listTable").DataTable({
		...
		"ajax": {
			'type': 'POST',
			'url': '/page/ajax/a_getCommunityList.php',
			'data': function() {
				return;
			}
		},
		...
		"drawCallback": function( settings ) {
			...
			_table.$('tr').on("click", function(e){
				if(e.srcElement.tagName=='TD'){
					...
					location.href="/?page=board_communityInfo&idx=" + data[0];
				}
			});	
		},
		...
		if(confirm("삭제하시겠습니까?")){
			...
			table.$('input[type="checkbox"]').each(function(){
				if($.contains(document, this)){
					if(this.checked){
						checkBoardIdxs+= $(this).val() + ",";
					}
				}
			});
			checkBoardIdxs = checkBoardIdxs.substring(0, checkBoardIdxs.length-1);

			$.post("/page/ajax/a_boardInfo.php", 
				{
					actionType: "ChkDeleteBBSInfo",
					board_idxs: checkBoardIdxs
				},function(d) {
					table.ajax.reload();
			});
		}
